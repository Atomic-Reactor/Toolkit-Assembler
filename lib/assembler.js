...
const getDirName = (file, source) => {
    let name = path.dirname(file);
    source.forEach((p) => { name = file.split(`${p}/`).join(''); });
...
module.exports = (params = {}) => {
    ...
    config.registry = _.flatten(source.map((p) => { return dirTree(p); }));
    ...
    // Get templates
    // -------------------------------------
    let tempFiles = dirTree(config.src.templates[0]);
    tempFiles.children.forEach((item) => {
    ...
    // Get themes
    // -------------------------------------
    let themeGlobs = _.flatten(config.src.themes.map((p) => { return [`${p}/**/*.scss`, `${p}/**/*.less`]; }));
    let themeFiles = globby.sync(themeGlobs, {nodir: true, nosort: true});
    themeFiles.forEach((file) => {
        let theme = getDirName(file, config.src.themes);
        let dir = file.split(`/${theme}/`).shift() + `/${theme}`;

        config.themes[theme] = config.themes[theme] || dirTree(dir);
    });
